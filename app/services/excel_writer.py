...
def write_rows_to_xlsx(rows: list[ExtractedRow], out_path: Path) -> Path:
    ...
    for i, r in enumerate(rows, start=2):
        ws.cell(row=i, column=1, value=r.sequence).alignment = body_alignment
        ws.cell(row=i, column=2, value=r.question_type.value).alignment = body_alignment
        ws.cell(row=i, column=3, value=_to_rich_text(r.question_text)).alignment = body_alignment
        ws.cell(row=i, column=4, value=r.answer_text).alignment = body_alignment
        ws.cell(row=i, column=5, value=r.branching_logic).alignment = body_alignment
